...
public class LessonControllerTest {
    ...
    class FindAll {
        ...
        @Test
        @DisplayName("should return 200 and list of lessons")
        void shouldReturn200WithListOfLessons() throws Exception {
            when(lessonService.findAllByModuleId(COURSE_ID, MODULE_ID)).thenReturn(List.of(lessonResponse()));

            mockMvc.perform(get("/courses/{courseId}/modules/{moduleId}/lessons", COURSE_ID, MODULE_ID))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$", hasSize(1)))
                    .andExpect(jsonPath("$[0].id").value(LESSON_ID.toString()));
        }

        @Test
        @DisplayName("should return 200 and empty list when module has no lessons")
        void shouldReturn200WithEmptyList() throws Exception {
            when(lessonService.findAllByModuleId(COURSE_ID, MODULE_ID)).thenReturn(List.of());
        ...
        void shouldReturn404WhenModuleDoesNotBelongToCourse() throws Exception {
            UUID id = UUID.randomUUID();
            when(lessonService.findAllByModuleId(COURSE_ID, id))
                    .thenThrow(new ResourceNotFoundException(
                            String.format(ErrorMessages.COURSE_NOT_FOUND, id)));
